...
class Balance():
    ...
    def __init__(self, config):
        ...
        self._env = balance()
        self._pendulum_limit = (-0.20*np.pi, 0.20*np.pi)     # alpha の範囲．これを超えたら倒立維持失敗．
        self._arm_limit = (-0.9*np.pi, 0.9*np.pi)            # theta の範囲．これを超えたら倒立維持失敗．
    ...
    def _digitized_state(self, obs):
        ...
        n_arm_rad = np.digitize(arm_rad, np.linspace(self._arm_limit[0], self._arm_limit[1], d+1)[1:-1])
        n_arm_vel = np.digitize(arm_vel.clip(-8, 8), np.linspace(-8, 8, d+1)[1:-1])
        n_pendulum_rad = np.digitize(pendulum_rad, np.linspace(self._pendulum_limit[0], self._pendulum_limit[1], d+1)[1:-1])
